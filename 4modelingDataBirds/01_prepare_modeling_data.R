...
r0 <- rast("../3birdRecordsProcess/records_species_final.tif")
r1 <- rast("../1predictorsEurope/biosTopos_europe_5min.tif")
r2 <- rast("../1predictorsEurope/land_proportions_europe_5min_final.tif")
r3 <- rast("../2fireSyndromes/db_pyromes_final.tif") # assuming this is the final pyromes raster name
r4 <- rast("../3birdRecordsProcess/samp_bias.tif")
...
# final filter: keep only rows where species records data exists 
# (using Corvus corone as an indicator of species data coverage)
final_modeling_data <- df_valid_pixels[!is.na(df_valid_pixels$Corvus_corone), ]
