bartHO_Eq <- function(records, explainVars, folds, ntree=200, k=2, power=2, base=0.95, name=NULL) {
  ...
  if(!dir.exists("random_pseudo_absences_macro_pyro")) dir.create("random_pseudo_absences_macro_pyro")
  if(!dir.exists("hyper_params_eval_macro_pyro")) dir.create("hyper_params_eval_macro_pyro")
  ...
  for (r in 1:nrep) {
    ...
    write.csv(get(paste0("randomPoint_Replication_", r)),
              file = paste0("random_pseudo_absences_macro_pyro/randomPseudoAbs_Replication_", r, "_", name, ".csv"))  
  }
  ...
  write.csv(out3, file = paste0("hyper_params_eval_macro_pyro/hyperEval_", name, "_macroPyros.csv"), row.names = FALSE)
}