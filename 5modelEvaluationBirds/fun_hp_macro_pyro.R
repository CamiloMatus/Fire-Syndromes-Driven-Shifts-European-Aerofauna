...
bartHO_Eq <- function(records, explainVars, folds, ntree=200, k=2, power=2, base=0.95, name=NULL) {
  ...
  if(file.exists("random_pseudo_absences_macro_pyro") == FALSE) {
    dir.create(file.path(getwd(), "random_pseudo_absences_macro_pyro"))
  }

  if(file.exists("hyper_params_eval_macro_pyro") == FALSE) {
    dir.create(file.path(getwd(), "hyper_params_eval_macro_pyro"))
  }
  ...
  for (r in 1:nrep) {
    ...
    write.csv(get(paste("randomPoint_Replication_", r, sep = "")),
              file = paste("random_pseudo_absences_macro_pyro/randomPseudoAbs_Replication_", r, "_", name, ".csv", sep = ""))  
  }
  ...
  write.csv(out3[, 2:15], file = paste("hyper_params_eval_macro_pyro/hyperEval", name, "_macroPyros.csv", sep = ""), row.names = F)
  write.csv(out3[, 2:15], file = paste("hyper_params_eval_macro_pyro/hyperEval", name, "_macroPyros.csv", sep = ""), row.names = F)
}