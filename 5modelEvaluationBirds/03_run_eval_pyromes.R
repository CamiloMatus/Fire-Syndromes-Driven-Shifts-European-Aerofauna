...
# source pyrome evaluation function
...
# calculate correlations including categorical pyrome variable
results <- correlation(env_vars, include_factors = TRUE, method = "auto")
filtered_results <- results %>% filter(str_detect(Parameter1, "pyrome") | str_detect(Parameter2, "pyrome"))

write.csv(data.frame(filtered_results), "tab_cor_predictors_pyro.csv", row.names = FALSE)

# create dummy variables and remove baseline consensus 0
env_vars_dummy <- dummy_cols(env_vars, 
                             select_columns = "pyrome_consensus", 
                             remove_selected_columns = TRUE)

env_vars_dummy <- env_vars_dummy[, !names(env_vars_dummy) %in% "pyrome_consensus_0"]
