...
bartHO_Eq <- function(records, explainVars, folds, ntree=200, k=2, power=2, base=0.95, name=NULL) {
  ...
  if(file.exists("random_pseudo_absences_baseline") == FALSE) {
    dir.create(file.path(getwd(), "random_pseudo_absences_baseline"))
  }

  if(file.exists("hyper_params_eval_baseline") == FALSE) {
    dir.create(file.path(getwd(), "hyper_params_eval_baseline"))
  }
  ...
  for (r in 1:nrep) {
    ...
    write.csv(get(paste("randomPoint_Replication_", r, sep = "")),
              file = paste("random_pseudo_absences_baseline/randomPseudoAbs_Replication_", r, "_", name, ".csv", sep = ""))  
  }
  ...
  for (h in 1:nrow(HyptoTest)) {
    ...
    for (run in 1:ncol(folds)) {  
      ...
      for (replication in 1:nrep) {
        ...
        # also in case a predictor column only has constant values
      ...
      if(nrow(dat.h.0[dat.h.0[,"folds...run."]==FALSE,]) != 0 | nrow(dat.h.0[dat.h.0[,"folds...run."]==FALSE,]) == 2) {
        ...
        if (length(pred.h) == 0) {
          ...
          if(nrep == 1) { pred.out <- pred.h } else {
            # mean predictions of replicated pseudo-absences
          ...
          # robust calculation of tss, kappa, sens and spec
  ...
  write.csv(out3[, 2:15], file = paste("hyper_params_eval_baseline/hyperEval", name, "_baseline.csv", sep = ""), row.names = F)
  write.csv(out3[, 2:15], file = paste("hyper_params_eval_baseline/hyperEval", name, "_baseline.csv", sep = ""), row.names = F)
}