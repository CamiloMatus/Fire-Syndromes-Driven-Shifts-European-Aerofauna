...
d_out <- d_out |>
    mutate(
        predictor_group = case_when(
            ...
            grepl("^macro_pyro|^pyrome_consensus", names) ~ "Fire Syndromes",
...
d_out <- d_out |>
    mutate(names_clean = case_when(
        ...
        grepl("^pyrome_consensus_", names) ~ sub("pyrome_consensus_", "P", names), 
        TRUE ~ names 
    ))
...
png("var_imp_combined_plot.png", width = 12, height = 7, units = 'in', res = 300)
...
write.csv(d_out, "./final_tabs/var_imp_birds.csv", row.names = FALSE)
...
write.csv(df_summary, "./final_tabs/var_imp_summary_birds.csv", row.names = FALSE)