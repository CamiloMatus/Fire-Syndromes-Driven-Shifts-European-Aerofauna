...
# update main db with dummy macro pyromes
db <- env_vars_dummy
...
for(spp in species_list) {
    ...
    for (replication in 1:nrep) {
        ...
        rp_path <- paste0("../05modelEvaluationBats/random_pseudo_absences_macro_pyro/randomPseudoAbs_Replication_", replication, "_", spp, ".csv")
        rp <- read.csv(rp_path)
