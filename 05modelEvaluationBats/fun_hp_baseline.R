...
bartHO_Eq <- function(records, explainVars, folds, ntree=200, k=2, power=2, base=0.95, name=NULL) {
  ...
  if(!dir.exists("random_pseudo_absences_baseline")) dir.create("random_pseudo_absences_baseline")
  if(!dir.exists("hyper_params_eval_baseline")) dir.create("hyper_params_eval_baseline")
  ...
  # defining random pseudo-absences
  ...
  for (r in 1:nrep) {
    ...
    write.csv(get(paste0("randomPoint_Replication_", r)),
              file = paste0("random_pseudo_absences_baseline/randomPseudoAbs_Replication_", r, "_", name, ".csv"))  
  }
  ...
  for (h in 1:nrow(HyptoTest)) {
    ...
    for (run in 1:ncol(folds)) {  
      ...
      message(paste("ntrees =", HyptoTest[h,"ntree"], "; k =", HyptoTest[h,"k"], "; base =",
                    HyptoTest[h,"base"], "; power =", HyptoTest[h,"power"],
                    "...", "combination", h, "of", nrow(HyptoTest)))
      ...
      pred.h <- c()
      for (replication in 1:nrep) {
        if(nrep != 1) message(paste("Replication", replication, "of", nrep))

        dat.h.0 <- rbind(dat.h0[dat.h0$records == 1, ],
                         dat.h0[row.names(dat.h0) %in% get(paste0("randomPoint_Replication_", replication)), ])
        ...
        if(nrow(dat.h.0[dat.h.0[, "folds...run."] == FALSE, ]) != 0 | nrow(dat.h.0[dat.h.0[, "folds...run."] == FALSE, ]) == 1) {
          if(!any(sapply(dat.h.0[dat.h.0[, "folds...run."] == TRUE, names(explainVars)], only_zeros))) {
            ...
            mod <- bart(x.train = dat.h.0[dat.h.0[, "folds...run."] == TRUE, names(explainVars)], 
                        ...
                        k = as.numeric(paste(HyptoTest[h,"k"])),
                        power = as.numeric(paste(HyptoTest[h,"power"])),
                        base = as.numeric(paste(HyptoTest[h,"base"])),
                        ntree = as.numeric(paste(HyptoTest[h,"ntree"])),
        ...
        if (length(pred.h) == 0) {
          ...
          if(length(unique(obs.h)) > 1 && !all(is.na(pred.out))) {
            ...
            for(t_idx in 1:100) {
              ...
              tp <- sum(obs.h == 1 & pred.out >= t_val, na.rm=TRUE)
              tn <- sum(obs.h == 0 & pred.out < t_val, na.rm=TRUE)
              fp <- sum(obs.h == 0 & pred.out >= t_val, na.rm=TRUE)
              fn <- sum(obs.h == 1 & pred.out < t_val, na.rm=TRUE)
      ...
      if(nrep != 1) message("Results of mean predictions")
      message(paste("auc =", round(auc.h, 2), "| boyce =", round(boyce.h, 2), "| tss =", round(tss.h, 2)))
    }   
  ...
  BOYCE.SD <- tapply(dat.out.f2$boyce, dat.out.f2$hp, sd, na.rm = TRUE)
  AUC <- tapply(dat.out.f2$auc, dat.out.f2$hp, mean, na.rm = TRUE)
  AUC.SD <- tapply(dat.out.f2$auc, dat.out.f2$hp, sd, na.rm = TRUE)
  TSS <- tapply(dat.out.f2$tss, dat.out.f2$hp, mean, na.rm = TRUE)
  TSS.SD <- tapply(dat.out.f2$tss, dat.out.f2$hp, sd, na.rm = TRUE)
  KAPPA <- tapply(dat.out.f2$kappa, dat.out.f2$hp, mean, na.rm = TRUE)
  KAPPA.SD <- tapply(dat.out.f2$kappa, dat.out.f2$hp, sd, na.rm = TRUE)
  ...
  out2 <- merge(x = out, y = dat.out.f2[, c(7:10, 13)], by = "hp", all.y = FALSE)
  out3 <- distinct(out2, .keep_all = TRUE)
  
  write.csv(out3[, 2:15], file = paste0("hyper_params_eval_baseline/hyperEval_", name, "_baseline.csv"), row.names = FALSE)
}